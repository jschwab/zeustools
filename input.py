...
### END ZEUS-MP2 NAMELIST TEMPLATE ###
    ...
    def write(self, filename = "zmp_inp"):
        ...
        with open(filename, "w") as zmpfile:
            
            # header (add jobname, etc ?)
            zmpfile.write(" #### ZEUSMP2 ZMP_INP AUTOGENERATED BY ZMPINP.PY #### \n")                       

            for namelist, optdict in self.namelists.iteritems():
            
                nopts = len(optdict)

                # open namelist
                if nopts == 0:
                    zmpfile.write("&{} /\n".format(namelist)) # empty options
                else:
                    zmpfile.write("&{}\n".format(namelist))
            
                for i, (option, value) in enumerate(optdict.iteritems()):
                
                    sep = ",\n"
                    if (i == nopts - 1): sep = "/\n"
                
                    zmpfile.write("   {0:10} = {1:22}{2:1}".format(option, _fmt_value(value), sep))
            


if __name__ == "__main__":
    z = ZeusMPInput()
    z.set_value("geomconf", "lgeom", 3)
    z.write()
